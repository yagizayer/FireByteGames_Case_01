...
namespace YagizAYER
{
    namespace Helper
    {
        ...
        public static class Extesions
        {
            ...
            /// <returns>List of Transforms</returns>
            public static List<Transform> GetAllChildren(this Transform me)
            {
                List<Transform> result = new List<Transform>();
                foreach (Transform child in me)
                    result.Add(child);
            ...
            {
                Vector3 rawMoveDir = Vector3.forward * conf.VerticalInput + Vector3.right * conf.HorizontalInput;

                Vector3 cameraForwardNormalized = Vector3.ProjectOnPlane(conf.CurrentCamera.forward, Vector3.up);
                Quaternion rotationToCamNormal = Quaternion.LookRotation(cameraForwardNormalized, Vector3.up);
                ...
                if (!Vector3.Equals(finalMoveDir, Vector3.zero))
                {
                    ...
                    me.transform.rotation = Quaternion.RotateTowards(me.transform.rotation, rotationToMoveDir, conf.RotationSpeed * Time.deltaTime);
                }
                float multipliedMovementSpeed = (conf.IsSprinting ? conf.SprintingSpeed : conf.MoveSpeed);

                Vector3 result = finalMoveDir * multipliedMovementSpeed * Time.deltaTime;
                me.Move(result);
